...
import { AuthDto, UpdateUserDto, DeleteUserDto } from './dto/user.dto';
import { ApiBearerAuth, ApiParam, ApiQuery, ApiTags } from '@nestjs/swagger';
import { JwtAuthGuard } from 'src/jwt-guard/jwt-guard.guard';
...
export class UserController {
	...
	@Get('updateToken')
	async updateToken(@Request() req: Request) {
		return this.userService.updateToken(req);
	}

	@ApiQuery({ required: false, type: [String] })
	@Get()
	async getUsers(@Query() query?: { idArray: string[] }) {
	...
	@UseGuards(JwtAuthGuard)
	@Put('update')
	async updateUser(@Body() body: UpdateUserDto, @Request() req: Request) {
	...
	@UseGuards(JwtAuthGuard)
	@Delete('delete')
	async deleteUser(@Body() body: DeleteUserDto, @Request() req: Request) {
